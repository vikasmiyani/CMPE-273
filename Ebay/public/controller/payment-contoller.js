...
app.controller('paymentController', function($scope,$window,$location,$http) {
		...
		console.log("inside setDetails"+isBuyItNow);
		if(isBuyItNow === 'true'){
			var sd_id = window.sd_id;
			var order_qty = window.order_qty;
			 $http.get('http://localhost:3000/fetchDetail',{params:{item_id:sd_id}})
				...
				for (var j = 0; j < $scope.item_list.length; j++) {
					if($scope.item_list[j] !== undefined){
	    				 total = total + $scope.item_list[j].subtotal;
	       				 total_item = total_item + Number($scope.item_list[j].ord_quantity);
	    				 }
				}
		...
		if(isValidCard && $scope.item_list.length > 0){
				$scope.isMissing = false;
					if(!isError){
						...
						var d = {items:arr,isBuyItNow:isBuyItNow};
	...
	$scope.removeItem = function(i){
		...
		$http.delete('http://localhost:3000/deleteShoppingCartItem/',{params: {id:id}})
		  .success(function (data) {
	          console.log("Success" + data);
	          if(data.success){
  	...
  	if(data.length > 0 && data[0] !== undefined){
		...
		var total_item = 0;
		 for (var i = 0; i < data.length; i++) {
	 		  ...
	 		  total_item = total_item +data[i].ord_quantity;
	 		  }
	 	  }
		 $scope.total = Math.round(total * 100)/100;
	 	  $scope.sales_tax = 4.29;	
