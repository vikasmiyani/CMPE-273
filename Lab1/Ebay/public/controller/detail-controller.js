...
app.controller('detailController', function($scope,$window,$location,$http, $interval) {
	...
	$scope.fetchDetail = function(){
		...
		var d = {item_id:item_id};
		 $http.get('http://localhost:3000/fetchDetail/',{params:{item_id:item_id}})
	      .success(function (data) {
	          ...
	          $scope.sd_id = detailData[0].sd_id;
	          $scope.item_id = detailData[0].id;
	          $scope.first_name = detailData[0].first_name;
	          $scope.city = detailData[0].city;
	          $scope.mysql_timestamp = detailData[0].bidding_due_time_stamp;
	         
//	          console.log(diffDays);
//	          $scope.diffDays = diffDays+  + ;
 	          console.log("sd_id"+detailData[0].sd_id);
	      })
	      .error(function(data) {
	...
	$scope.placeBid = function(){
		...
		if( $scope.diffDays > 0){
			if(bid_price > current_bidding_price){
				...
				var sd_id = $scope.sd_id;
				var d = {item_id:item_id,sd_id:sd_id,bidding_price:bid_price};
	...
	$scope.reload = function(){
		  $window.location.href = "http://localhost:3000/detailPg/"+$scope.sd_id;
	};
