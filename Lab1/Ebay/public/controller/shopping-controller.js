...
app.controller('shoppingController', function($scope,$window,$location,$http,$document) {
	...
	$scope.update = function(i){
		...
		var qty = this.qty;
		var sd_qty = $scope.shoppingItem_list[i].sd_qty;
		if(qty > 0 && qty !== undefined){
			  $scope.isError = false;
			if(qty <= sd_qty){
				  $scope.isError = false;
				var sc_id = $scope.shoppingItem_list[i].sc_id;
				var quantity = qty;
	...
	$scope.removeItem = function(i){
		...
		var id = $scope.shoppingItem_list[i].sc_id;
		var d = {id:id};
